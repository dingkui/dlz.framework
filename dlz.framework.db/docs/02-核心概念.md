...
```java
// 查询
DB.Pojo.select(User.class)          // Wrapper 查询（推荐）
DB.Pojo.select(user)              // Wrapper 查询（推荐）
...
| 组件名称          | 结果类型     | 说明                               |
|---------------|----------|----------------------------------|
| **Condition**  | 条件构造器    | 查询到多条时取第一条,取不到结果返回空<br/>支持多条结果报错 |
...
```java
ResultMap result = DB.Pojo.select("user").eq("id", 1).query();
...
// 分页结果
Page<User> result = DB.Pojo.select(User.class).page(page).page();
...
### 日志输出流程
```java
DB.Pojo.select(User.class).eq(User::getId, 1).list();
//输出日志如下：
