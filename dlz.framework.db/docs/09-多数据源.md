...
// 切换到从库查询
DB.Dynamic.use("slave").query(User.class).list();
...
// 主库写入
DB.Dynamic.use("master").insert(user).execute();
```
...
public class OrderService {
    ...
    public void createOrder(Order order) {
        // 主库写订单
        DB.Dynamic.use("master").insert(order).execute();
        
        // 报表库写统计（不在同一事务！）
        DB.Dynamic.use("report").insert(stats).execute();
    }
}
```
