...
```java
// 查询全部
List<User> all = DB.Pojo.select(User.class).list();
...
```java
// 查询单条
ResultMap result = DB.Pojo.select("user")
    .eq("id", 1)
    .queryOne();
// 查询单条
User user = DB.Pojo.select("user")
    .eq("id", 1)
    .queryOne(User.class);

// 查询列表
List<ResultMap> list = DB.Pojo.select("user")
    .eq("status", 1)
    .queryList();

// 转换为 Bean
List<User> users = DB.Pojo.select("user")
    .eq("status", 1)
...
List<ResultMap> list = DB.Pojo.select(tableName)
    .eq("status", 1)
    .queryList();
```
