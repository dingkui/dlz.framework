...
```java
// ❌ 查询全部字段
DB.Wrapper.query(User.class).list();
...
```java
// ❌ N+1 问题
List<Order> orders = DB.Wrapper.query(Order.class).list();
...
Map<Long, User> userMap = DB.Wrapper.query(User.class)
...
public List<Order> findMyOrders() {
    ...
    return DB.Wrapper.query(Order.class)
