...
// 批量插入
int count = DB.insertBatch(users).excute();
...
// 方式1：循环更新
users.forEach(user -> {
    DB.update(user).eq(User::getId, user.getId()).excute();
});
...
// ❌ 避免循环单条操作
for (User user : users) {
    DB.insert(user).excute();  // 每次都是独立连接
}

// ✅ 使用批量操作
DB.insertBatch(users).excute();  // 一次连接，批量执行
```
---
