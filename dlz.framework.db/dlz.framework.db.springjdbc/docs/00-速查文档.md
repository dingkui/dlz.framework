...
## 目录
[一、查询操作](#一查询操作)
- [1.1 基于 Bean 的查询（推荐）](#11-基于-bean-的查询推荐)
- [1.2 基于表名的查询](#12-基于表名的查询)
- [1.3 原生 JDBC 查询](#13-原生-jdbc-查询)
- [1.4 预设 SQL 查询（Key-SQL）](#14-预设-sql-查询key-sql)

[二、条件构造器](#二条件构造器)
- [2.1 基础条件](#21-基础条件)
- [2.2 范围查询(IN,between查询)（多种写法）](#22-范围查询inbetween查询多种写法)
- [2.3 模糊检索](#23-模糊检索)
- [2.3 OR / AND 嵌套条件](#23-or--and-嵌套条件)
- [2.4 复杂嵌套示例](#24-复杂嵌套示例)
- [2.5 自定义 SQL 片段 (Apply/Sql)](#25-自定义-sql-片段-applysql)
- [2.6 空值自动忽略](#26-空值自动忽略)
...
[四、插入操作](#四插入操作)
- [4.1 Bean 插入](#41-bean-插入)
- [4.2 指定表名插入](#42-指定表名插入)

[五、更新操作](#五更新操作)
- [5.1 Bean 更新](#51-bean-更新)
- [5.2 指定表名更新](#52-指定表名更新)
- [5.3 复杂条件更新](#53-复杂条件更新)

[六、删除操作](#六删除操作)
- [6.1 Bean 删除（自动逻辑删除）](#61-bean-删除自动逻辑删除)
- [6.2 条件删除](#62-条件删除)
- [6.3 安全机制：无条件删除更新保护](#63-安全机制无条件删除更新保护)

[七、预设 SQL（Key-SQL）](#七预设-sqlkey-sql)
- [7.1 配置预设 SQL](#71-配置预设-sql)
- [7.2 使用预设 SQL](#72-使用预设-sql)
- [7.3 预设 SQL 嵌套](#73-预设-sql-嵌套)
...
// 查询数量
long count = DB.Wrapper.query(User.class)
    .eq(User::getStatus, 1)
    .count();
```
...
```java
// 基础分页
Page<User> page = DB.Wrapper.query(User.class)
    .eq(User::getStatus, 1)
...
// 插入并返回自增主键
Long id = DB.Wrapper.insert(user).insertWithAutoKey();
```
...
DB.Wrapper.update(User.class).set("status", 1).execute();
...
```java
// 方括号内的条件：参数为空时自动忽略
SqlKeyQuery query = DB.Sql.select("key.test.user.find");
...
// 切换数据源
DBDynamic.use("test",()-> {
    return DB.Wrapper.getById(SysSql.class, "1");
});
...
public void transfer(Long fromId, Long toId, BigDecimal amount) {
    DB.Wrapper.update(Account.class)
...
### 多数据源事务控制
```java
DBDynamic.useTransactional("default",()-> {
        return DB.Wrapper.getById(SysSql.class, "1");
});
```
