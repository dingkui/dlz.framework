...
// 方式2：返回 ResultMap
ResultMap result = DB.Wrapper.query(User.class)
    .eq(User::getId, 1)
    .query();
...
```java
// 查询全部
List<User> all = DB.Wrapper.query(User.class).list();
...
```java
List<String> cities = DB.Wrapper.query(User.class)
    .select(User::getCity)
    .queryStrList(String.class);


String citie = DB.Wrapper.query(User.class)
        .select(User::getCity)
        .ed(User::getId,1)
...
ResultMap result = DB.Wrapper.query("user")
    ...
    .query();
// 查询单条
User user = DB.Wrapper.query("user")
    .eq("id", 1)
    .query(User.class);
...
// 转换为 Bean
List<User> users = DB.Wrapper.query("user")
    .eq("status", 1)
    .list(User.class);
```
...
List<ResultMap> list = DB.Wrapper.query(tableName)
    .eq("status", 1)
    .list();
```
