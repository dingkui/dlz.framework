...
String citie = DB.query(User.class)
        .select(User::getCity)
        .ed(User::getId,1)
...
```java
// 单个结果：ResultMap
ResultMap result = DB.jdbcSelect("select * from user where id = ?",1)
        .query();
...
```java
//sql 预设到xml：key.user.getUserWithDept
List<ResultMap> list = DB.sqlSelect("key.user.getUserWithDept")
        .para("status", 1)
        .para("type", "tech")
        .queryList();
```
...
DB.insert("user").value(data).excute();
```
...
[//]: # ()
[//]: # (DB.saveBatch&#40;"user", dataList&#41;.excute&#40;&#41;;)
...
DB.update(user).eq(User::getId, 1).excute();
...
DB.delete("user").where(where).excute();
...
```java
// 执行删除
DB.delete(User.class).eq(User::getId, 1).excute();
