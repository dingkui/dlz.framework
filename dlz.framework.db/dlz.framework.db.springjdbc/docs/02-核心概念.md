...
```java
// 查询
DB.Wrapper.query(User.class)          // Wrapper 查询（推荐）
DB.Wrapper.query(user)              // Wrapper 查询（推荐）
...
```java
ResultMap result = DB.Wrapper.query("user").eq("id", 1).query();
...
// 分页结果
Page<User> result = DB.Wrapper.query(User.class).page(page).page();
...
### 日志输出流程
```java
DB.Wrapper.query(User.class).eq(User::getId, 1).list();
//输出日志如下：
